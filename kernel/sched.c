...
// Defining Struct here for mailbox
struct msg_struct
{	
	char * string;		// It stores original string
	int length;		// It stores length
	pid_t from;		// It stores the sender pid
	pid_t to;		// It stores the receiver pid
	struct list_head linked_list;
};


// two lock which 
DECLARE_MUTEX_LOCKED(lock_it);
DECLARE_MUTEX_LOCKED(unlock_it);


asmlinkage long sys_mysend(pid_t pid, int n, char* buf)
{       
        struct task_struct *p;
        for (p = &init_task ; (p = next_task(p)) != &init_task ;)  // go through each running process ID
        {       
                if (p -> pid == current->pid)
                        {	
				printk("Someone called mysend PID: %d\n",pid);		

				// store from user space to kernel space in a struct
				struct msg_struct *  user_pointer_to_kernel;
        			user_pointer_to_kernel = kmalloc(n, GFP_KERNEL);
				copy_from_user(user_pointer_to_kernel->string, buf, n);
				user_pointer_to_kernel->length = n;
				user_pointer_to_kernel->to = pid;
				user_pointer_to_kernel->from = p->pid;
				INIT_LIST_HEAD(&user_pointer_to_kernel->linked_list);
				printk("Done with creating struck on kernel\n");	
				up(&lock_it);
				down(&unlock_it);
				return 0;
...
{
	down(&lock_it);
	printk("Someone called myreceive PID: %d\n",pid);
	struct list_head *tmp;
	struct msg_struct * msg_received;
	
	printk("tell mysend to release lock\n");	
	...
	struct task_struct *p;
        for (p = &init_task ; (p = next_task(p)) != &init_task ;)  // go through each running process ID
        {
                if (p -> pid == current->pid)
		{		
			msg_received = list_entry(tmp, struct msg_struct, linked_list);
		
			// retrive the message from the struct if the matching pid found in sender	
			if(msg_received->to == current->pid){
			int length = msg_received->length;
			up(&unlock_it);
			return length;
			}
		}
